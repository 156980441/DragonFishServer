...
<div class="pageHeader">
	<form id="viewAdvertiseListForm" onsubmit="return navTabSearch(this);" action="/business/viewAdvertiseList" method="post">
	<div class="searchBar">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="/business/viewAddAdvertiseInfo?SEQ=19900114" target="navTab" rel="viewAddAdvertiseInfo"><span>添加</span></a></li>
			<li><a class="edit" href="/business/viewAddAdvertiseInfo?SEQ={SEQ}" target="navTab" rel="viewAddAdvertiseInfo" ><span>修改</span></a></li>
			<li><a class="delete" href="/login/deleteAdvertiseInfo?SEQ={SEQ}" target="ajaxTodo" rel="deleteAdvertiseInfo"><span>删除</span></a></li>
		</ul>
	</div>
	<table class="table" width="100%" layoutH="138">
		<thead>
			<tr>
				<th width="80">No</th>
				<th width="120" orderField="TITLE" <c:if test='${page.orderField == "TITLE"}'>class="${page.orderDirection}"</c:if>>广告标题</th>
				<th width="80">广告位置</th>
		...
		<tbody>
			<c:forEach items="${viewAdvertiseList}" var="item" varStatus="i">
				<tr target="SEQ" rel="${item.SEQ }">
					<td>${i.count + (page.pageNum - 1) * page.numPerPage}</td>
					<td>${item.TITLE }</td>
					<td>
						<c:if test="${item.LAYOUT eq 1 }">位图广告</c:if>
						<c:if test="${item.LAYOUT eq 0 }">链接广告</c:if>
					</td>
